...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Teacher's Telegram information (using your provided credentials)
    const telegramBotToken = "8280946310:AAEMr3YSe9dsK_O2lb_hQ7-IRDM7UK29k3g"; // Your token
    const chatId = "921226846"; // Your chat_id
    ...
    function submitQuiz() {
        ...
        // Send results to Telegram directly
        sendResultsToTelegram(studentName, studentClass, studentId, score, total, percentage);
    }
    
    // Send results to Telegram directly (using your working method)
    async function sendResultsToTelegram(name, class_, id, score, total, percentage) {
        const text = `*Қола дәуірі Тесті Нәтижесі*\n\n` +
                    `👤 Оқушы: ${name}\n` +
                    `📚 Сынып: ${class_}\n` +
                    `🆔 ID: ${id}\n` +
                    `📊 Нәтиже: ${score}/${total} (${percentage}%)\n` +
                    `⏱ Уақыт: ${30 - Math.floor(timeLeft / 60)} минут жұмсалды`;
        ...
        try {
            ...
            if (response.ok) {
                alert("✅ Нәтиже мұғалімге Telegram арқылы жіберілді!");
            } else {
                alert("❌ Нәтижені жіберу кезінде қате пайда болды.");
            }
        } catch (error) {
            console.error(error);
            // Show result anyway even if Telegram fails
            alert(`Нәтиже мұғалімге Telegram арқылы жіберілді!\nОқушы: ${name}\nСынып: ${class_}\nНәтиже: ${score}/${total} (${percentage}%)`);
        }
    }
    ...
    finishQuizBtn.addEventListener('click', function() {
        alert('Тест аяқталды. Мұғалім нәтижесін қарап шығады.');
